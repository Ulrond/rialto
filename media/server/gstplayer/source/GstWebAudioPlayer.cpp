...
uint32_t GstWebAudioPlayer::writeBuffer(uint8_t *mainPtr, uint32_t mainLength, uint8_t *wrapPtr, uint32_t wrapLength)
{
    ...
    bool success = m_context.writeBufferCond.wait_for(lock, std::chrono::milliseconds(kMaxWriteBufferTimeoutMs),
                                                       [this, initialCompletionCounter]() {
                                                           return m_context.writeCompletionCounter !=
                                                                  initialCompletionCounter;
                                                       });
    if (!success)
    {
